...
namespace SoftBlueBD
{
    public partial class Uklady : Form
    {
        ...
        private void CreateSQLCommand()
        {
            ...
            try
            {
                foreach (string line in System.IO.File.ReadLines(@"Układy Scalone.ini"))                                   //dla każdego elemntu wpisanego w pliku konfiguracyjnym
                {
            ...
            {
                MessageBox.Show("Nie odnaleziono pliku konfiguracyjnego: Układy Scalone.ini");                             //reakcja na błąd w otwarciu pliku konfiguracyjnego
            }
        ...
        private void Uklady_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void LabelTEXT()
        {
            ...
            for (int j = 0; j < advancedDataGridView1.Rows.Count; ++j)
            {
                ...
                sum += Convert.ToInt32(advancedDataGridView1.Rows[j].Cells[8].Value);                  //cells 10 bo tam jest właśnie kolumna ilość
            }
            int i = 0;
            foreach (string item in WalutaComboBox.Items)
            {
                ...
                for (int j = 0; j < advancedDataGridView1.Rows.Count; ++j)
                {
                    if (Convert.ToString(advancedDataGridView1.Rows[j].Cells[10].Value) == item)
                        try
                        {
                            kasa[i] += Convert.ToDecimal(advancedDataGridView1.Rows[j].Cells[9].Value) * Convert.ToDecimal(advancedDataGridView1.Rows[j].Cells[8].Value);
                        }
                        catch (Exception)
                        {
                        }
